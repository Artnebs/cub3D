...
{
    printf("[parse_map_line] Entrée, cub3d=%p, cub3d->garbage=%p\n", (void*)cub3d, (void*)cub3d->garbage);
    char    **new_map;
    int     i;

    printf("[parse_map_line] Allocation du nouveau tableau de map (taille: %d)...\n", cub3d->map.height + 2);
    new_map = gc_malloc(cub3d, sizeof(char *) * (cub3d->map.height + 2));
    if (!new_map)
    {
        printf("[parse_map_line] Erreur d'allocation new_map\n");
        return (0);
    }
    i = 0;
    ...
    printf("[parse_map_line] Allocation de la nouvelle ligne (longueur: %zu)...\n", ft_strlen(line) + 1);
    new_map[i] = gc_malloc(cub3d, ft_strlen(line) + 1);
    if (!new_map[i])
    {
        printf("[parse_map_line] Erreur d'allocation new_map[i]\n");
        gc_free(cub3d, new_map);
        return (0);
    }
    printf("[parse_map_line] Copie de la ligne dans new_map[i]...\n");
    ft_strlcpy(new_map[i], line, ft_strlen(line) + 1);
    new_map[i + 1] = NULL;
    if (cub3d->map.map)
    {
        printf("[parse_map_line] Libération de l'ancienne map...\n");
    ...
    {
        printf("[parse_map_line] Nouvelle largeur de map: %zu\n", ft_strlen(line));
        cub3d->map.width = (int)ft_strlen(line);
    }
    printf("[parse_map_line] Fin OK\n");
    return (1);
}

static int  parse_line(t_cub3d *cub3d, char *line)
{
    printf("[parse_line] Ligne reçue: '%s'\n", line);
    // Nettoyage du retour à la ligne final
    ...
    if (line[0] == 'N' && line[1] == 'O' && line[2] == ' ')
        ...
        return (parse_color(cub3d, line, &cub3d->map.ceiling_color));
             ...
             line[0] == 'S' || line[0] == 'E' || line[0] == 'W' || line[0] == ' ')
...
int parse_map(t_cub3d *cub3d, const char *filename)
{
    ...
    printf("Début de la boucle de lecture...\n");
    while ((line = get_next_line(fd)))
    {
        printf("Ligne lue: %s\n", line);
        if (!parse_line(cub3d, line))
        {
            printf("Erreur dans parse_line\n");
            free(line);
            close(fd);
            return (0);
        }
