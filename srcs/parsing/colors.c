...
/*   Updated: 2025/07/23 16:24:45 by anebbou          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	parse_rgb_values(t_cub3d *cub3d __attribute__((unused)), char *str, int *color)
{
	char	**parts;
	int		r;
	int		g;
...
int	parse_color_line(t_cub3d *cub3d, char *line, int *color)
{
	...
	if (parse_rgb_values(cub3d, tokens[1], color) == -1)
	{
		free_split(tokens);
