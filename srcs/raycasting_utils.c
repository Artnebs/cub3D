...
void	cast_ray(t_cub3d *cub3d, int x)
{
	...
	calculate_wall_distance(cub3d, &data.params, &data.perp_wall_dist, data.side);
	...
	setup_wall_rendering(cub3d, &data.params, &data.wall, &data.wr_params);
	draw_floor_ceiling(cub3d, x, data.draw_params.draw_start, data.draw_params.draw_end);
